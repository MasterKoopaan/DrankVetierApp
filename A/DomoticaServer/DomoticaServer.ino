...
void loop()
{
   ...
   while (ethernetClient.connected()) 
   {
      ...
      sensorValue2 = readSensor(1, 100);         // update sensor value
...
void switchKaku(byte unit, bool state) {
  apa3Transmitter.sendUnit(unit, state);          // APA3 Kaku (Gamma)                
  delay(100);
  stateKaku[unit] = state;
  Serial.print("kaku switch "); Serial.print(unit); Serial.print(" to "); Serial.println(stateKaku[unit]);
}
...
{     
         ...
         case 'b': // Report sensor value to the app  
            intToCharBuf(sensorValue2, buf2, 4);                // convert to charbuffer
            server.write(buf2, 4);                             // response is always 4 chars (\n included)
            Serial.print("Sensor 2: "); Serial.println(buf2);
            break;
         case 'd':
            switchKaku(0, !stateKaku[0]);
            break;
         case 'e':
            switchKaku(1, !stateKaku[1]);
            break;
         case 'f':
            switchKaku(2, !stateKaku[2]);
            break;
